...
namespace Vu {
    struct VuAssetLoader {
        static void LoadGltf(std::filesystem::path path, VuMesh& dstMesh) {
        ...
        std::span<uint8> normalSpanByte = dstMesh.vertexBuffer.getSpan(dstMesh.getNormalOffsetAsByte(), sizeof(float3) * dstMesh.vertexCount);
        std::span<float3> normalSpan = std::span(reinterpret_cast<float3 *>(normalSpanByte.data()), dstMesh.vertexCount);

        std::span<uint8> tangentSpanByte = dstMesh.vertexBuffer.getSpan(dstMesh.getTangentOffsetAsByte(), sizeof(float4) * dstMesh.vertexCount);
