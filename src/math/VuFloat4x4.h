...
namespace Vu::Math {
    ...
    struct Float4x4 {
        ...
        Float4 getColumn(int col) const;

        // Set column from Float4
        void setColumn(int col, const Float4& vec);
    ...
    Float3 operator*(const Float4x4& mat, const Float3& vec);
    ...
    Float4x4 createTranslation(const Float3& position);

    // Creates a scaling matrix
    Float4x4 createScale(const Float3& scale);

    // Creates a rotation matrix from quaternion
    Float4x4 createRotation(const Quaternion& quaternion);

    // Creates a TRS (Translation-Rotation-Scale) matrix
    Float4x4 createTRSMatrix(const Float3& position, const Quaternion& quaternion, const Float3& scale);
