...
readFile(const std::filesystem::path& path) {
  if (std::filesystem::exists(path) == false) { return std::nullopt; }
...
inline void
createFile(const std::filesystem::path& path, std::span<const uint8_t> data) {
  ...
  if (!file) { throw std::runtime_error("Failed to write to file: " + path.string()); }
}
