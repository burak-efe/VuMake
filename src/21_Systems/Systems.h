...
inline void drawMesh(Transform& transform, const MeshRenderer& meshRenderer)
{
...
inline void spinn(Transform& trs, const Spinn& spin)
{
...
inline void drawSpinUI(uint32_t elemID, Spinn& spinn)
{
    ...
    {
        ImGui::SliderFloat(std::format("Radians/perSecond##{0}", elemID).c_str()
                           , &spinn.angle, 0.0f, 32.0f);
    }
...
inline void cameraFlySystem(Transform& trs, Camera& cam)
{
